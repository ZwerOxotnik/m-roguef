...
local function c_() global.p.character.destructible=true global.p.character.die() game.print({"rf.c"}) end
...
local function gui()
	...
	--mastery
	local g=b.gui.left
	g.add{type="frame",name="main",direction="vertical"} -- it was style="outer_frame_style"
	g.main.add{type="sprite-button",name="mastery",sprite="item/stage",tooltip={"gui.mastery"}} -- it was ,style="side_menu_button_style"
	g.main.add{type="table",name="mt",column_count=1,colspan=3}
	for i=1,7 do
		for j=1,3 do
			g.main.mt.add{type="sprite-button",name=i.."-"..j,sprite="item/mastery-"..i.."-"..j,tooltip={"mastery-info."..i.."-"..j}} -- it was ,style="side_menu_button_style"
	...
	--enemy health
	local top = b.gui.top
	top.add{type="frame",name="main",direction="vertical"} -- it was style="outer_frame_style"
	top.main.add{type="frame",name="enemy",direction="horizontal"} -- it was style="outer_frame_style"
	local enemy_gui = top.main.enemy
	enemy_gui.add{type="progressbar",name="bar",size=1,value=0} -- it was ,style="health_progressbar_style"
...
local function distance(ent,ents)
	local a
	local b
	local c
	local d
...
script.on_event(defines.events.on_player_created, function(event)
	...
	local new_force = game.create_force(tostring(a))
	local enemy_force = game.forces.enemy
	global.p.force=new_force
...
script.on_event("reload", function(event)
	local b=global.p
	if b.get_inventory(defines.inventory.character_guns)[1].valid_for_read then
		local n=b.get_inventory(defines.inventory.character_guns)[1].name
		b.get_inventory(defines.inventory.character_ammo)[1].set_stack({name="ammo-"..string.sub(n,string.find(n,"-")+1,string.len(n)),1000})
	end
	if b.get_inventory(defines.inventory.character_guns)[2].valid_for_read then
		local n=b.get_inventory(defines.inventory.character_guns)[2].name
		b.get_inventory(defines.inventory.character_ammo)[2].set_stack({name="ammo-"..string.sub(n,string.find(n,"-")+1,string.len(n)),1000})
	end
	if b.get_inventory(defines.inventory.character_guns)[3].valid_for_read then
		local n=b.get_inventory(defines.inventory.character_guns)[3].name
		b.get_inventory(defines.inventory.character_ammo)[3].set_stack({name="ammo-"..string.sub(n,string.find(n,"-")+1,string.len(n)),1000})
	end
...
script.on_event("useitem", function(event)
	...
	if global.active[1] and global.active[2]==global.active[3] then
		...
		global.active[2]=0
		-- b.get_quickbar()[6].clear()  -- TODO: FIX THIS
	end
end)
...
script.on_event(defines.events.on_gui_click, function(event)
	...
	--mastery button
	if n=="mastery" and b.gui.left.main.mt.visible==true then
		b.gui.left.main.mt.visible=false
	elseif n=="mastery" and b.gui.left.main.mt.visible==false then
		b.gui.left.main.mt.visible=true
...
script.on_event("interaction", function(event)
	...
	if b.get_inventory(defines.inventory.character_guns)[b.character.selected_gun_index].valid_for_read then
		local n=b.get_inventory(defines.inventory.character_guns)[b.character.selected_gun_index].name
...
script.on_event(defines.events.on_player_main_inventory_changed, function(event)
	local b=global.p
	if b.get_inventory(defines.inventory.character_main).get_item_count("money") > 0 then
		-- b.get_quickbar()[7].set_stack({name="money",count=b.get_item_count("money")})  -- TODO: FIX THIS
		b.get_inventory(defines.inventory.character_main).remove({name="money",count=b.get_inventory(defines.inventory.character_main).get_item_count("money")})
	else
		local list={"active","level","xp","stage"}
		for i,j in pairs(list) do
			if b.get_inventory(defines.inventory.character_main).get_item_count(j) > 0 then
				b.get_inventory(defines.inventory.character_main).remove({name=j,count=b.get_inventory(defines.inventory.character_main).get_item_count(j)})
			end
		end
	end
	if b.get_inventory(defines.inventory.character_main).get_item_count("heal") > 0 then
		...
		b.get_inventory(defines.inventory.character_main).remove({name="heal",count=1})
	end
	for i=1,global.gunnum do
		b.get_inventory(defines.inventory.character_main).remove({name="ammo-"..i})
	end
end)
...
--opening
	...
	elseif event.tick<=21*60 then
		if game.get_entity_by_tag("train").train.passengers[1]~=b.character then
			game.get_entity_by_tag("train").train.passengers[1]=b.character
		end
		for i=1,20 do
...
--every tick call
	...
	if b.character.health/b.character.prototype.max_health<0.1 and passive(12) then
		b.character.health=b.character.health+b.character.prototype.max_health*0.3
		-- b.get_quickbar().remove({name="passive-12",count=1})  -- TODO: FIX THIS
		sound(b.position,"firstaid")
		b.surface.create_entity{name="firstaid",position=b.position}
	end
			...
			elseif global.stage.num==4 then
				...
				elseif a.a and a.a.valid then
					...
					if a.a and a.a.valid then
						...
						enemy_bar.visible=true
						enemy_bar.value=a.a.health/a.a.prototype.max_health
						enemy_bar.label.caption={"gui.boss-4",math.ceil(a.a.health),math.ceil(a.a.prototype.max_health),math.ceil(a.a.health/a.a.prototype.max_health*100)}
					end
				elseif a.a and a.a.valid==false then
					stageclear(global.stage.num)
				end
			...
			elseif global.stage.num==6 then
				...
				elseif a.g and b.position.x<131 then
					...
					if b.position.x<131 and b.position.y>190 then
						...
						local enemy_bar = global.p.gui.top.main.enemy.bar
						enemy_bar.visible=true
						enemy_bar.value=1-(b.position.x/130)
						enemy_bar.label.caption={"gui.boss-6",math.ceil(b.position.x),130,math.ceil(b.position.x/130*100)}
					end
	...
	if event.tick%60==17 then
		local player_inventory = b.get_inventory(defines.inventory.character_armor)
	...
	if event.tick%(60*60)==2773 then
		-- TODO: FIX AND CHECK: or b.character.prototype.resistances["damage-enemy"]
		if (event.tick>global.dodge and b.character.active and b.character.destructible==false) or b.character.prototype.max_health~=1000 or b.character.prototype.healing_per_tick>0 then
			c_()
		end
	end
end
...
script.on_event(defines.events.on_trigger_created_entity, function(event)
	...
	local n
	if b.get_inventory(defines.inventory.character_guns)[b.character.selected_gun_index].valid_for_read then
		n=b.get_inventory(defines.inventory.character_guns)[b.character.selected_gun_index].name
