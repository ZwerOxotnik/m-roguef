if script.level.level_name ~= "roguef" then return end
...
script.on_event(defines.events.on_gui_click, function(event)
	...
	elseif main_mt_gui and main_mt_gui[n] then
		...
		if x > -1 and x < 40 and y > -7 and y < 20 then
			...
			else main_mt_gui[i.."-2"].style="red_circuit_network_content_slot_style"
				main_mt_gui[i.."-3"].style="red_circuit_network_content_slot_style"
			end
		else
...
script.on_event("interaction", function(event)
	...
	if ent and ent.valid and get_distance(character, ent) < 2 then
		...
		elseif ent.name=="player-test" and global.water and global.water~=0 and global.test and global.test[1] and global.test[1].valid then
			surface.create_entity{name = "playertext", position = global.test[1].position, text = {"rf.water-2"}, color={g=1}}
			global.water=false
		else
			for i=1,2 do
...
do
	...
	script.on_event(defines.events.on_built_entity, function(event)
		...
		if ent.name ~= "rf_no" then return end
...
--every tick call
	...
	if weapons[14] then
		if weapons[14][2]<event.tick and (event.tick-weapons[14][2])%2==0 then
			...
			if n==10 then weapons[14]=false end
		end
	end
	--weapon 15
	if weapons[15] then
	...
	if weapons[25] and weapons[25].valid then
		...
		local d=surface.find_entities_filtered{area={{pos.x-0.5,pos.y-0.5},{pos.x+0.5,pos.y+0.5}},type="projectile"}
		for i, j in pairs(d) do
			if j==weapons[25] then table.remove(d,i) end
		end
			...
			elseif global.stage.num==4 then
				...
				elseif boss4 and boss4.valid then
					...
					elseif health_ratio>0.8 then
						if a.c==false then a.c=event.tick a.d=player.position end
						if event.tick%45==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,180+(event.tick-a.c)),duration=2}
					elseif health_ratio>0.7 then
						if a.e==false then a.e=event.tick a.d=player.position a.c=false end
						if event.tick%40==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,90+(event.tick-a.e)),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,-90+(event.tick-a.e)),duration=2}
					elseif health_ratio>0.6 then
						if a.c==false then a.c=event.tick a.d=player.position a.e=false end
						if event.tick%35==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,60+(event.tick-a.c)),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,180+(event.tick-a.c)),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,300+(event.tick-a.c)),duration=2}
					elseif health_ratio>0.5 then
						if a.e==false then a.e=event.tick a.d=player.position a.c=false end
						if event.tick%30==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,180-(event.tick-a.e)*1.5),duration=2}
					elseif health_ratio>0.4 then
						if a.c==false then a.c=event.tick a.d=player.position a.e=false end
						if event.tick%25==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,90-(event.tick-a.c)*1.5),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,-90-(event.tick-a.c)*1.5),duration=2}
					elseif health_ratio>0.3 then
						if a.e==false then a.e=event.tick a.d=player.position a.c=false end
						if event.tick%20==0 then a.b=tile4(a.b) end
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,60-(event.tick-a.e)*1.5),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,180-(event.tick-a.e)*1.5),duration=2}
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,300-(event.tick-a.e)*1.5),duration=2}
					...
					elseif health_ratio>0 then
						...
						surface.create_entity{name="beam-4",position=boss4_pos,source=boss4,target=targetrotate(boss4_pos,targetline_pos,180+(event.tick-a.c-1)*72),duration=6}
					end
			...
			elseif global.stage.num==5 then
				...
				elseif a.a and a.a>0 then
					...
					if a.a<=240-(24*2) then--80%
						if a.c%180==45 then
							surface.create_entity{name="biter-normal-1",
					...
					if a.a<=240-(24*6) then--40%
						if a.c%180==135 then
							surface.create_entity{name="spitter-normal-1",
								position=targetrotate(a.t.position,{x=a.t.position.x,y=a.t.position.y-3},a.t.orientation*360+90)
							}
						end
					end
					...
					if a.g[3].is_opened() and a.g[4].is_opened()==false then
						...
						elseif a.c and a.e and (a.c.valid or a.e.valid) then
							if a.e and a.e.valid and event.tick%5==0 then
								movement()
								local pos
								local ents=surface.find_entities_filtered{area={{a.e.position.x-5,a.e.position.y-5},{a.e.position.x+5,a.e.position.y+5}},type="projectile"}
								if #ents>0 then
									local ent=false
									for i,j in pairs(ents) do
										if ent==false then ent=j
										elseif get_distance(a.e,ent)>get_distance(a.e,j) then ent=j
										end
									end
							...
							if a.c and a.c.valid then
								...
								surface.create_entity{name="beam-4",position=a.c.position,source=a.c,target=targetrotate(a.c.position,targetline(a.c.position,{x=a.c.position.x,y=a.c.position.y-1},30),-(event.tick-a.d)),duration=2}
							end
					...
					if a.g[5].is_opened() and a.g[6].is_opened()==false then
						...
						elseif a.c and (a.c.valid or a.d.valid or a.e.valid or a.f.valid or a.h.valid) then
							...
							if a.c.valid and a.c.name=="biter-normal-6" then
								...
								surface.create_entity{name="beam-4",position=a.c.position,source=a.c,target=targetrotate(a.c.position,targetline(a.c.position,{x=a.c.position.x,y=a.c.position.y-1},30),-(event.tick-a.i)),duration=2}
							elseif a.c.valid and a.c.name=="spitter-normal-6" and event.tick%5==0 then
								...
								if #ents>0 then
									...
									for _, j in pairs(ents) do
										if ent==false then ent=j
										elseif get_distance(a.c,ent)>get_distance(a.c,j) then ent=j
										end
									end
			...
			elseif global.stage.num==7 then
				...
				--phase 2
					...
					if a.a and a.a.valid and a.d>300 and (a.d-300)%(30-math.ceil(a.speed*100))==0 and a.d<3300 then
						for i=(20-a.speed*100),360,(20-a.speed*100) do
							surface.create_entity{name="ep-30",
				...
				--phase 4
					...
					if a.d%400==0 then
						sound(player.position,"anda-cast-1")
						for i=15,360,15 do
							surface.create_entity{name="ep-50",
					...
					elseif a.d%400>230 and a.d%400<=320 and a.d%4==0 then
						surface.create_entity{name="ep-7-1",position=targetline(a.a.position,player.position,(a.d%400-230)*0.3)}
					end
				...
				--phase 5
					...
					elseif a.a and a.a.valid and a.d%800==400 then
						sound(player.position,"anda-cast-1")
						for i=10,360,10 do
							surface.create_entity{name="ep-50",position=targetrotate(a.a.position,targetline(a.a.position,player.position,1.2),i),target=targetrotate(a.a.position,targetline(a.a.position,player.position,30),i),speed=0.01}
						end
					elseif a.a and a.a.valid and a.d%800==600 then
						sound(player.position,"anda-cast-1")
						surface.create_entity{name="slowdown-4-0",position=a.a.position,target=a.a}
					elseif a.a and a.a.valid and a.d%800>630 and a.d%800<=720 and a.d%4==0 then
						surface.create_entity{name="ep-7-1",position=targetline(a.a.position,player.position,(a.d%800-630)*0.3)}
			...
			elseif global.stage.num==8 then
				...
				--phase 1
					...
					elseif a.d>220 and a.d<=220+60*20 and (a.d-220)%60==0 then
						...
						surface.create_entity{name="blaze-3",position=targetline(a.p[1],a.p[4],(a.d-220)/60*get_distance(a.p[1],a.p[4])/20)}
					...
					elseif a.d>1480 and a.d<=1480+60*20 and (a.d-1480)%60==0 then
						...
						surface.create_entity{name="blaze-3",position=targetline(a.p[4],a.p[2],(a.d-1480)/60*get_distance(a.p[4],a.p[2])/20)}
					...
					elseif a.d>2740 and a.d<=2740+60*20 and (a.d-2740)%60==0 then
						...
						surface.create_entity{name="blaze-3",position=targetline(a.p[2],a.p[5],(a.d-2740)/60*get_distance(a.p[2],a.p[5])/20)}
					...
					elseif a.d>4000 and a.d<=4000+60*20 and (a.d-4000)%60==0 then
						...
						surface.create_entity{name="blaze-3",position=targetline(a.p[5],a.p[3],(a.d-4000)/60*get_distance(a.p[5],a.p[3])/20)}
					...
					elseif a.d>5260 and a.d<=5260+60*20 and (a.d-5260)%60==0 then
						...
						surface.create_entity{name="blaze-3",position=targetline(a.p[3],a.p[1],(a.d-5260)/60*get_distance(a.p[3],a.p[1])/20)}
				...
				--phase 2
					...
					elseif a.d%1920>460 and a.d%1920<=580 and a.d%5==0 then
						...
						if a.a and a.a.valid then surface.create_entity{name="explosion-8",position=a.a.position} end
					...
					elseif a.d%1920==1160 then
						for i=1,5 do
							for j=30,360,30 do
								if a.a and a.a.valid then surface.create_entity{name="ep-7-1",position=targetrotate(a.p[6],targetline(a.p[6],a.p[7],i),j)} end
							end
						end
				...
				--phase 3
					...
					elseif a.d>220 and a.d<=220+60*20 and (a.d-220)%60==0 then
						...
						surface.create_entity{name="blaze-8",position=targetline(a.p[1],a.p[4],(a.d-220)/60*get_distance(a.p[1],a.p[4])/20)}
					elseif a.d==220+60*20+60 then
						blood(a.p[4],3)
						surface.create_entity{name="worm-normal-8-2",position=a.p[4]}
					...
					elseif a.d>1480 and a.d<=1480+60*20 and (a.d-1480)%60==0 then
						...
						surface.create_entity{name="blaze-8",position=targetline(a.p[4],a.p[2],(a.d-1480)/60*get_distance(a.p[4],a.p[2])/20)}
					elseif a.d==1480+60*20+60 then
						blood(a.p[2],3)
						surface.create_entity{name="worm-normal-8-3",position=a.p[2]}
					...
					elseif a.d>2740 and a.d<=2740+60*20 and (a.d-2740)%60==0 then
						...
						surface.create_entity{name="blaze-8",position=targetline(a.p[2],a.p[5],(a.d-2740)/60*get_distance(a.p[2],a.p[5])/20)}
					elseif a.d==2740+60*20+60 then
						blood(a.p[5],3)
						surface.create_entity{name="worm-normal-8-4",position=a.p[5]}
					...
					elseif a.d>4000 and a.d<=4000+60*20 and (a.d-4000)%60==0 then
						...
						surface.create_entity{name="blaze-8",position=targetline(a.p[5],a.p[3],(a.d-4000)/60*get_distance(a.p[5],a.p[3])/20)}
					elseif a.d==4000+60*20+60 then
						blood(a.p[3],3)
						surface.create_entity{name="worm-normal-8-5",position=a.p[3]}
					...
					elseif a.d>5260 and a.d<=5260+60*20 and (a.d-5260)%60==0 then
						...
						surface.create_entity{name="blaze-8",position=targetline(a.p[3],a.p[1],(a.d-5260)/60*get_distance(a.p[3],a.p[1])/20)}
					elseif a.d==5260+60*20+60 then
						a.b=4
						a.c=event.tick
						surface.create_entity{name="item-on-ground",position=a.p[0],stack={name="heal"}}
			...
			elseif global.stage.num==9 then
				...
				--phase 1
					...
					elseif a.e[a.i-1] and a.c>250 and a.c<850 and a.c%15==0 then
						surface.create_entity{name="blaze-8",position=targetline(a.e[a.i-1].position,a.e[a.i].position,math.ceil((a.c-250)/15)-1)}
					elseif a.c==850 then
						a.p[2]=player.position
					elseif a.c>850 and a.c<=1210 then
						a.ep[a.c-850]=surface.create_entity{name="ep-50",position=targetrotate(a.p[0],targetline(a.p[0],a.p[2],30),a.c-850),target=a.p[0],speed=0.3}
					...
					if a.d==1 and a.a and a.a.valid then
						surface.create_entity{name="beam-4",position=a.a.position,source=a.a,target=targetrotate(a.a.position,targetline(a.a.position,a.p[1],20),180-(event.tick-a.b)),duration=2}
					end
				elseif a.d==2 then
				--phase 2
					a.c=event.tick-a.b
					...
					elseif a.c==420 then
						...
						a.e[6]=surface.create_entity{name="boss-7",position=targetrotate(a.p[0],targetline(a.p[0],player.position,10),180)}
					...
					elseif a.c>420 then
						...
						elseif a.c%1200>360 and a.c%1200<=720 then
							a.ep[a.c%1200-360]=surface.create_entity{name="ep-50",position=targetrotate(a.p[0],targetline(a.p[0],a.p[2],30),a.c%1200-360),target=player.position,speed=0.3}
							a.ep[a.c%1200-360].active=false
						elseif a.c%1200>720 and a.c%1200<=1080 and a.ep[a.c%1200-720].valid then
							surface.create_entity{name="ep-50",position=a.ep[a.c%1200-720].position,target=player.position,speed=0.3}
						...
						if a.e[6] and a.e[6].valid then
							...
							elseif a.c>600 and a.c%600>30 and a.c%600<=120 and a.c%4==0 then
								surface.create_entity{name="ep-7-1",position=targetline(a.e[6].position,player.position,(a.c%600-30)*0.3)}
							end
						end
						if a.d==2 and a.a and a.a.valid then
							surface.create_entity{name="beam-4",position=a.a.position,source=a.a,
				...
				--phase 3
					...
					elseif a.c==420 then
						...
						blood(a.e[5].position,3)
						a.e[7]=surface.create_entity{name="boss-8-1",position=targetrotate(a.p[0],targetline(a.p[0],player.position,10),180)}
					...
					elseif a.c>420 then
						...
						elseif a.c%1200>760 and a.c%1200<=1120 then
							a.ep[a.c%1200-760]=surface.create_entity{name="ep-50",position=targetrotate(a.p[0],targetline(a.p[0],a.p[2],30),a.c%1200-760),target=player.position,speed=0.3}
						...
						if a.e[6] and a.e[6].valid then
							...
							elseif a.c>1200 and a.c%1200>330 and a.c%1200<=420 and a.c%4==0 then
								surface.create_entity{name="ep-7-1",position=targetline(a.e[6].position,player.position,(a.c%1200-330)*0.3)}
							end
						end
						if a.d==3 and a.a and a.a.valid then
							local targetline_pos = targetline(a.a.position,a.p[1],20)
							surface.create_entity{name="beam-4",position=a.a.position,source=a.a,target=targetrotate(a.a.position,targetline_pos,60+a.c),duration=2}
							surface.create_entity{name="beam-4",position=a.a.position,source=a.a,target=targetrotate(a.a.position,targetline_pos,180+a.c),duration=2}
							surface.create_entity{name="beam-4",position=a.a.position,source=a.a,target=targetrotate(a.a.position,targetline_pos,300+a.c),duration=2}
						end
					end
				elseif a.d==4 then
				--phase 4
					...
					elseif a.c>300 and a.c<=660 then
						a.ep[a.c-300]=surface.create_entity{name="ep-200",position=targetrotate(a.p[0],targetline(a.p[0],a.p[2],30),a.c-300),target=player.position,speed=get_distance(targetrotate(a.p[0],targetline(a.p[0],a.p[2],30),a.c-300),player.position)/200}
					...
					elseif a.g==960 then
						...
						player.set_ending_screen_data({"boss.9-end",g[1][1],g[1][2],g[2][1],g[2][2],g[3][1],g[3][2],g[4][1],g[4][2],g[5][1],g[5][2],g[6][1],g[6][2],g[7][1],g[7][2],g[8][1],g[8][2],math.ceil((game.tick-global.timer)/60*100)/100})
	...
	if event.tick%60==17 then
		...
		if global.active[1] and global.active[2]~=global.active[3] then
			...
			if global.active[2]==global.active[3] then
				surface.create_entity{name = "weapon-text", position = {player.position.x,player.position.y+0.5}, text = "active ready", color={r=1,g=1,b=1}}
			end
			if global.active[2]>global.active[3] then
				global.active[2]=global.active[3]
			end
	...
	if event.tick%(60*60)==2773 then
		...
		if (event.tick>global.dodge and character.active and character.destructible==false) or character.prototype.max_health~=1000 or character.prototype.healing_per_tick>0 then
			c_()
		end
	end
end)
...
script.on_event(defines.events.on_trigger_created_entity, function(event)
	...
	if ent.name=="player-fire" then
		...
		if n=="weapon-13" then
			...
			else global.weapons[13]=global.weapons[13]+0.1
				local d=global.weapons[13]
				if d>=1 and d<1.1 then
					player.surface.create_entity{name = "weapon-text", position = {player.position.x,player.position.y+0.5}, text = "33%", color={r=1,g=1,b=1}}
				elseif d>=2 and d<2.1 then
					player.surface.create_entity{name = "weapon-text", position = {player.position.x,player.position.y+0.5}, text = "66%", color={r=1,g=1,b=1}}
				elseif d>=3 and d<3.1 then
					player.surface.create_entity{name = "weapon-text", position = {player.position.x,player.position.y+0.5}, text = "100%", color={r=1,g=1,b=1}}
				elseif d>3 then
					d=3
				end
			end
